...
class Auth extends MY_Controller {
	...
	function index()
	{
		...
		else
		{
			...
			//Jeśli user nie jest zalogowany oraz nie jest adminem to przenosimy na strone główną
			//To jest domyślna akcja po wpisaniu auth
	...
	function login()
	{
		if($this->config->item('login_active', 'ion_auth') === TRUE)
		{
			if (!$this->ion_auth->logged_in()) //if user is logged in
			{
				...
				else
				{
					...
					$this->show_view('auth/login', $this->data);
				}
			}
			else
			{
				//redirect to the home page because user is logged in now
				redirect('/', 'refresh');
			}
		}
		else
	...
	//register - added by Kimmy
	function register()
	{
		//Check if register is active/opened
		if($this->config->item('register_active', 'ion_auth') === TRUE)
		{
			...
			$this->form_validation->set_rules('Email', $this->lang->line('register_user_validation_email_label'), 'required|valid_email');
	...
	//			$this->load->view('auth/auth_footer');
				echo $this->load->view('pages/register', $this->data);
			}
	...
	function change_password()
	{
		...
		if ($this->form_validation->run() == false)
		{
			...
			$this->show_view('auth/change_password', $this->data);
		}
		else
		{
	...
	function forgot_password()
	{
		...
		if ($this->form_validation->run() == false)
		{
			...
			//$this->load->view('templates/footer');
			$this->show_view('auth/forgot_password', $this->data);
		}
		else
		{
	...
	public function reset_password($code = NULL)
	{
		...
		if ($user)
		{
			...
			if ($this->form_validation->run() == false)
			{
				...
				//$this->load->view('templates/footer');
				$this->show_view('auth/reset_password', $this->data);
			}
			else
			{
