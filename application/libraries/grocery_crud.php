...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showMoreForm($state_info)
	{
		...
		$this->_theme_view('more.php',$data);
		$this->_inline_js("var js_date_format = '".$this->js_date_format."';");
	}
...
return (object)array('output' => $this->views_as_string, 'js_files' => $js_files, 'css_files' => $css_files, 'extra' => $extra);
	...
	protected function get_hint_input($field_info,$value)
	{
		$this->set_css($this->default_css_path.'/jquery_plugins/autocomplete.css');
		$this->set_js($this->default_javascript_path.'/jquery_plugins/jquery.autocomplete.min.js');						$value = !is_string($value) ? '' : str_replace('"',"&quot;",$value); 
		$extra_attributes = '';
		if(!empty($field_info->db_max_length))
			$extra_attributes .= "maxlength='{$field_info->db_max_length}'"; 
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value=\"$value\" $extra_attributes />";		
		if(!empty($field_info->extras) && is_array($field_info->extras))		{
			$hints = array_unique($field_info->extras);
			
			$input .= "<script type=\"text/javascript\">
		$(function(){  var autocomplete_data = [";
			
			foreach($hints as $hint)
			{
				$input .= "{ value: '$hint', data: '$hint'},";
			}
			
			$input .= "];
				$('#field-{$field_info->name}').autocomplete({
	...
	/*protected function get_more_input_fields($field_values = null)
	{
		$fields = $this->get_edit_fields();
		$types 	= $this->get_field_types();
...
class grocery_CRUD_States extends grocery_CRUD_Layout
{
	...
	protected function getMoreUrl($primary_key = null)
	{
		if($primary_key === null)
			return $this->state_url('more');
		else
			return $this->state_url('more/'.$primary_key);
	}
	...
	public function getStateInfo()
	{
		...
		switch ($state_code) {
			...
			case 18:
				...
				{
					throw new Exception('On the state "more" the Primary key cannot be null', 6);
					die();
				}
				break;
...
class grocery_CRUD extends grocery_CRUD_States
{
	...
	//protected function get_more_fields()
	//{
	//	if($this->edit_fields_checked === false)
	//	{
	//		$field_types = $this->get_field_types();
